        ...
        private IEnumerable<Member> ParseFunctions()
        {
            ...
            yield return new Function(this, "Parse")
            {
                ...
                Code = new[]
                {
                        ...
                        $"return new {Name}({Components.ForIndexUpTo(i => $"{BaseTypeName}.Parse(kvp[{i}].Trim())").CommaSeparated()});"
                    },
            ...
            if (BaseType.Name != "bool")
            {
                yield return new Function(this, "Parse")
                {
                    ...
                    Code = new[]
                    {
                            "var kvp = s.Split(new[] { sep }, StringSplitOptions.None);",
                            string.Format("if (kvp.Length != {0}) throw new FormatException(\"input has not exactly {0} parts\");", Components),
                            $"return new {Name}({Components.ForIndexUpTo(i => $"{BaseTypeName}.Parse(kvp[{i}].Trim(), provider)").CommaSeparated()});"
                        },
                ...
                yield return new Function(this, "Parse")
                {
                    ...
                    Code = new[]
                    {
                            "var kvp = s.Split(new[] { sep }, StringSplitOptions.None);",
                            string.Format("if (kvp.Length != {0}) throw new FormatException(\"input has not exactly {0} parts\");", Components),
                            $"return new {Name}({Components.ForIndexUpTo(i => $"{BaseTypeName}.Parse(kvp[{i}].Trim(), style)").CommaSeparated()});"
                        },
                ...
                yield return new Function(this, "Parse")
                {
                    ...
                    Code = new[]
                    {
                            "var kvp = s.Split(new[] { sep }, StringSplitOptions.None);",
                            string.Format("if (kvp.Length != {0}) throw new FormatException(\"input has not exactly {0} parts\");", Components),
                            $"return new {Name}({Components.ForIndexUpTo(i => $"{BaseTypeName}.Parse(kvp[{i}].Trim(), style, provider)").CommaSeparated()});"
                        },
            ...
            {
                Static = true,
                ParameterString = $"string s, out {Name} result",
            ...
            {
                Static = true,
                ParameterString = $"string s, string sep, out {Name} result",
                Code = new[]
                {
                        "result = Zero;",
                        ...
                        $"result = ok ? new {Name}({CompArgString}) : Zero;",
                        "return ok;"
                    },
            ...
            if (!BaseType.IsBool)
            {
                ...
                {
                    Static = true,
                    ParameterString = $"string s, string sep, NumberStyles style, IFormatProvider provider, out {Name} result",
                    Code = new[]
                    {
                            "result = Zero;",
                            "if (string.IsNullOrEmpty(s)) return false;",
