    ...
    internal class ComponentWiseStaticFunction : Member
    {
        ...
        public override IEnumerable<Member> GlmMembers()
        {
            ...
            yield return new Function(ReturnType, Name)
            {
                Static = true,
                Comment = Comment,
                Parameters = ap.Concat(ParameterNames.Select((p, i) => ParameterTypes[i].Name + " " + p)),
                CodeString = $"{OriginalType.Name}.{Name}({ap2 + ParameterNames.CommaSeparated()})"
            };
                ...
                CanScalar0 && CanScalar1 && CanScalar2)
            {
                yield return new Function(ReturnType.BaseType ?? ReturnType, Name)
                {
                    Static = true,
                    Comment = Comment,
                    Parameters = ap.Concat(ParameterNames.Select((p, i) => (ParameterTypes[i].BaseType ?? ParameterTypes[i]).Name + " " + p)),
